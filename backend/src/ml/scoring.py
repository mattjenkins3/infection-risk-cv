...
class RiskScorer:
    ...
    def score(self, signals: FeatureSignals) -> RiskResult:
    ...
    def _explanation(self, details: List[SignalDetail], level: str) -> str:
        top = sorted(details, key=lambda item: item.weight * item.value, reverse=True)[:2]
        cues = ", ".join(detail.name.replace("_", " ") for detail in top)
        return (
            f"Estimated risk level: {level}. The strongest visual cues were {cues}. "
